...
export default function Dashboard({ metrics, teams = [], recentBookings = [], analytics = {}, operations = {}, facility }) {
    ...
    const displayName = auth?.user?.name?.trim().replace(/(?:\s+|^)admin$/i, '').trim() || 'Admin';
    const data = {
            ...
            <div className="flex flex-col flex-1 w-full gap-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch">
                    ...
                    <div className="lg:col-span-8 relative rounded-3xl overflow-hidden flex flex-col justify-between min-h-[260px] sm:min-h-[300px] xl:min-h-[340px]">
                        ...
                        <div className="relative z-10 px-5 sm:px-6 pt-1 sm:pt-2 pb-5 sm:pb-6 flex flex-col h-full justify-between">
                            <h1 className="max-w-full break-words text-2xl sm:text-3xl font-medium leading-snug text-white">
                                {displayName}
                            </h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch">
                    ...
                    <GlassCard className="lg:col-span-4 min-h-[320px] grayscale bg-black">
                        <div className="p-4 sm:p-5 h-full flex flex-col">
                            <div className="flex items-center gap-2 mb-1"><Activity className="w-4 h-4 text-white" /><h3 className="text-sm font-bold text-white">Operations Snapshot</h3></div>
                            <p className="text-xs text-gray-400 mb-4">Live records that need attention</p>
                            <div className="grid grid-cols-2 gap-2.5 mb-5">
                                <div className="bg-white/[0.06] border border-white/[0.08] rounded-2xl p-3"><Users className="w-4 h-4 text-white mb-2" /><p className="text-xl font-bold text-white">{operations.clients ?? 0}</p><p className="text-[10px] text-gray-400">Total clients</p></div>
                                <div className="bg-white/[0.06] border border-white/[0.08] rounded-2xl p-3"><Crown className="w-4 h-4 text-white mb-2" /><p className="text-xl font-bold text-white">{operations.active_memberships ?? 0}</p><p className="text-[10px] text-gray-400">Active members</p></div>
                                <div className="bg-white/[0.06] border border-white/[0.08] rounded-2xl p-3"><AlertTriangle className="w-4 h-4 text-white mb-2" /><p className="text-xl font-bold text-white">{operations.low_stock ?? 0}</p><p className="text-[10px] text-gray-400">Low stock items</p></div>
                                <div className="bg-white/[0.06] border border-white/[0.08] rounded-2xl p-3"><ShieldCheck className="w-4 h-4 text-white mb-2" /><p className="text-xl font-bold text-white">{operations.open_tickets ?? 0}</p><p className="text-[10px] text-gray-400">Open tickets</p></div>
                            </div>
                            <div className="mt-auto">
                                ...
                                <div className="space-y-2">
                                    {bookingStatuses.map((status) => {
                                        ...
                                        return <div key={status.label} className="flex items-center gap-2"><span className="w-2 h-2 rounded-full" style={{ backgroundColor: '#ffffff' }} /><span className="text-[11px] text-gray-400 w-16">{status.label}</span><div className="h-1.5 flex-1 bg-white/10 rounded-full overflow-hidden"><div className="h-full rounded-full" style={{ width: `${(status.value / total) * 100}%`, backgroundColor: '#ffffff' }} /></div><span className="text-[11px] text-white font-semibold w-6 text-right">{status.value}</span></div>;
                                    })}
                                </div>
                            </div>
                        </div>
